...
async def setup_browser(browser: str = "chrome") -> str:
    ...
    if system == "Windows":
        ...
        # Check binary — try alternate Program Files path for Chrome
        if not Path(binary).exists():
            alt = binary.replace("Program Files\\", "Program Files (x86)\\")
            if Path(alt).exists():
                binary = alt
            else:
                return _err(
                    f"Could not find {browser} at:\n  {binary}\n\n"
                    "Is it installed? Adjust the path in the .bat file if needed."
                )
        ...
        return _ok({
            ...
            "next_steps": [
                ...
                f"2. Double-click '{bat_name}' on your Desktop — Chrome opens with your logins ready",
